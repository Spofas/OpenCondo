...
export const createContact = withAdmin(async (ctx, input: ContactInput) => {
...
export const updateContact = withAdmin(async (ctx, contactId: string, input: ContactInput) => {
  const parsed = contactSchema.safeParse(input);
  if (!parsed.success) {
    return { error: parsed.error.issues[0].message };
  }
...
export const deleteContact = withAdmin(async (ctx, contactId: string) => {
  const supplier = await db.supplier.findFirst({
    where: { id: contactId, condominiumId: ctx.condominiumId },
  });
  if (!supplier) return { error: "Contacto não encontrado" };

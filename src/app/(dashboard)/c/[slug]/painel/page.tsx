...
async function AdminDashboardContent({
  ...
  if (openMaintenanceCount > 0) {
    attentionItems.push({
      ...
      href: `/c/${slug}/comunicacao/manutencao`,
      cta: "Ver pedidos",
    });
  }
    ...
    <>
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-2">
          <QuickActionButton href={`/c/${slug}/financas/quotas`} icon={CreditCard} label="Registar pagamento" />
          <QuickActionButton href={`/c/${slug}/financas/despesas`} icon={Receipt} label="Nova despesa" />
          <QuickActionButton href={`/c/${slug}/comunicacao/avisos`} icon={Megaphone} label="Criar aviso" />
        </div>
      </section>
    </>
  );
  ...
  return (
    <div className="max-w-2xl">
      ...
      <Suspense fallback={<DashboardSkeleton isAdmin={isAdmin} />}>
        {isAdmin ? (
          <AdminDashboardContent condoId={condoId} userId={session.user.id!} slug={slug} />
        ) : (
          <MemberDashboardContent condoId={condoId} userId={session.user.id!} slug={slug} />
        )}
      </Suspense>
    </div>
  );

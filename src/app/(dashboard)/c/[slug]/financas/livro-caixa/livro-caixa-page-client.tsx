...
export function LivroCaixaPageClient({
  ...
  async function handleSetOpeningBalance(data: OpeningBalanceInput) {
    const result = await setOpeningBalance(condominiumId, data);
    if (result.error) {
  ...
  async function handleAddAdjustment(data: AdjustmentInput) {
    const result = await addAdjustment(condominiumId, data);
    if (result.error) {
  ...
  async function handleDeleteAdjustment(id: string) {
    const result = await deleteAdjustment(condominiumId, id);
    if (!result.error) {
      setDeletingId(null);
      router.refresh();
    }
